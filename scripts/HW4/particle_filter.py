...
class ParticleFilter(object):
    ...
    def transition_update(self, u, dt):
        ...
        num_controls = u.shape[0]
        eps = np.random.multivariate_normal(mean=np.zeros(num_controls), cov=self.R * dt, size=self.M) # Gaussian input noise
        # print(eps.shape)
        us = eps + u[None,:]
    ...
    def resample(self, xs, ws):
        ...
        M = xs.shape[0]
        u = np.sum(ws) * (np.linspace(r, r + 1., M, False))
        idx = np.searchsorted(np.cumsum(ws), u)
        self.xs = xs[idx]
        self.ws = ws[idx]
        ########## Code ends here ##########
...
class MonteCarloLocalization(ParticleFilter):
    ...
    def transition_model(self, us, dt):
        ...
        ########## Code ends here ##########
        M = us.shape[0]
        g = np.zeros(shape=(M, 3))
        for m in range(M):
            x = self.xs[m, :]
            u = us[m, :]
            g[m, :] = tb.compute_dynamics(x, u, dt, compute_jacobians=False)
        return g
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        for i in range(I):
            ...
            for j in range(hs.shape[0]):
                h = hs[j]
                vs_ji = np.zeros_like(h.T)
                vs_ji[:, 0] = angle_diff(z[0], h[0])
                vs_ji[:, 1] = z[1] - h[1]
                distances = np.array([np.matmul(v.T, np.matmul(Q_inv, v)) for v in vs_ji])
                vs[j, i] = vs_ji[np.argmin(distances)]
        ########## Code ends here ##########
    ...
    def compute_predicted_measurements(self):
        ...
        for i, particle in enumerate(self.xs):
            h = np.zeros_like(self.map_lines)
            for j in range(self.map_lines.shape[1]):
                l = tb.transform_line_to_scanner_frame(self.map_lines[:,j], particle, self.tf_base_to_camera, False)
                h[:,j] = tb.normalize_line_parameters(l)
            hs[i] = h
        ########## Code ends here ##########
