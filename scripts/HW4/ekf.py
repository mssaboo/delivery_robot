...
class EkfLocalization(Ekf):
    ...
    def measurement_model(self, z_raw, Q_raw):
        ...
        #vstack all elements of v_list to compute z using concatenate along axis = 0
        z = np.concatenate(v_list)
        #compute Q as given in HINT
        Q = scipy.linalg.block_diag(*Q_list)
        #v_stack all elements of H_list to compute H using concatenate along axis = 0
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        #compute I
        I = z_raw.shape[1]
        # init v_list, Q_list, H_list arrays 
        v_list = []
        Q_list = []
        H_list = []
        #validation gate
        validation_gate = self.g*self.g 
        #for each observed line 
        for i in range(I):
            #set min_dist = validation_gate so the constraint mentioned in problem is always satisfied
            min_dist = validation_gate
            #select z and Q
            z = z_raw[:,i]
            Q = Q_raw[i]
            #variable to check if v_new, Q_new, H_new are updated for a particular i
            isUpdated = False
            J = hs.shape[1]
            #for each map entry
            for j in range(J):
                #equations mentioned in pset
                h_j = hs[:,j]
                #use angle_diff for delta_alpha
                delta_alpha = angle_diff(z[0],h_j[0])
                delta_r = z[1] - h_j[1]
                #compute v_ij
                v_ij = np.array([delta_alpha,delta_r])
                #compute S_ij
                S_ij = np.matmul(np.matmul(Hs[j],self.Sigma),Hs[j].T) + Q
                #compute d_ij
                S_ij_inv = np.linalg.inv(S_ij)
                d_ij = np.matmul(np.matmul(v_ij.T,S_ij_inv),v_ij)
                #update v_new, Q_new, H_new if distance is less than current min_dist and set isUpdated = True
                if d_ij < min_dist:
                    v_new = v_ij
                    Q_new = Q
                    H_new = Hs[j]
            ...
            #if isUpdated -> add v_new, Q_new, H_new to corresponding lists
            if isUpdated:
                v_list.append(v_new)
                Q_list.append(Q_new)
                H_list.append(H_new)
...
class EkfSlam(Ekf):
    ...
    def measurement_model(self, z_raw, Q_raw):
        ...
        # fixed to use code from p1 instead of re-implementing
        #vstack all elements of v_list to compute z
        z = np.concatenate(v_list)
        #compute Q as given in HINT
        Q = scipy.linalg.block_diag(*Q_list)
        #v_stack all elements of H_list to compute H
        H = np.concatenate(H_list)
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        for i in range(I):
            ...
            #variable to check if v_new, Q_new, H_new are updated for a particular i
            isUpdated = False
            J = (self.x.size - 3) // 2 #update to use state self.x instead of map_lines
            #for each map entry
            for j in range(J):
                #equations mentioned in pset
                h_j = hs[:,j]
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(J):
            ...
            h, Hx_full = tb.transform_line_to_scanner_frame(np.array([alpha, r]), self.x[:3], self.tf_base_to_camera) #alpha and r replace self.map_lines
            Hx[:,:3] = Hx_full[:, :3]

            # compute the partial derivative dh_r/d_alpha as x_cam*np.sin(alpha) + y_cam*np.cos(alpha)
            # adapted from tb.transform_line_to_scanner_frame()
            theta = self.x[2]
            rot_mtx_b2w = np.array([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]])
            x_cam, y_cam = self.x[:2] + rot_mtx_b2w @ self.tf_base_to_camera[:2]
            dhr_dalpha = x_cam*np.sin(alpha) - y_cam*np.cos(alpha) 
            ...
            if j >= 2:
                Hx[:,idx_j:idx_j+2] = np.array([[1,0], [dhr_dalpha, 1]])
